...
# Глобальное что бы обращаться из другой функции

tk = Tk() #обєкт пайтон для GUI
tk.iconbitmap('pino.ico')

app_running = True #булева функція для перевірки активності программи

size_canvas_x = 500
size_canvas_y = 500
# Фіксировані розмір ігрового поля (шаг 1)

s_x = s_y = 9  # Розмір ігрового поля > 8 і менше 18
#s_y = 8

step_x = size_canvas_x // s_x  # шаг по горизонтали цел. дел., размер шагов между ячейками
step_y = size_canvas_y // s_y  # шаг по вертикали

# корректеровка для цел. дел.,
...
# ships generations and they count and lenght

ships = s_x // 2  # максимальне число кораблів
ship_len1 = s_x // 5  # довжина першого корабля (макс)
ship_len2 = s_x // 3
ship_len3 = s_x // 2
enemy_ships1 = [[0 for i in range(s_x + 1)] for i in range(s_y + 1)] # динамическое определение списка (если кораблей нет, там 0), матрица єл. списка
enemy_ships2 = [[0 for i in range(s_x + 1)] for i in range(s_y + 1)] # для врага, местоположения кораблей

# print(enemy_ships2)

list_ids = []  # список обєктів canvas

# points1 - список куди ми нажами мишкою
points1 = [[-1 for i in range(s_x)] for i in range(s_y)]
points2 = [[-1 for i in range(s_x)] for i in range(s_y)]

# boom - список уражень кораблів противника
boom = [[0 for i in range(s_x)] for i in range(s_y)] # знач попадання по противнику, для перевірки перемоги, при кожному кліку перевірка переможця

# ships_list - список кораблів гравця 1 и гравця 2
ships_list = []

# hod_igrovomu_polu_1 - if true, hid igroka №2, else go player №1
...
def on_closing():
    global app_running
    if messagebox.askokcancel("Вийти з гри", "Бажаєте вийти с гри?"):
        app_running = False
        tk.destroy()


tk.protocol("WM_DELETE_WINDOW", on_closing) # шаг 2, что будет при закрытии
tk.title("Гра Морскій Бій")
tk.resizable(0, 0) # незя менять size = false
...
canvas.create_rectangle(size_canvas_x + menu_x, 0, size_canvas_x + menu_x + size_canvas_x, size_canvas_y,
                        fill="lightyellow") # прямоугольник белого цвета белого цвета
canvas.pack() # zip
tk.update() # update


def draw_table(offset_x=0): # малюєм ігрове поле
...
draw_table() # визиваємо функцію

draw_table(size_canvas_x + menu_x)

t0 = Label(tk, text="Гравець №1", font=("Helvetica", 16)) # текст подписи игроков с указаными координатами
...
t0.configure(bg="aqua") #back ground
...
def button_show_enemy1(): # пробег для отображения кораблей красным
    for i in range(0, s_x):
        for j in range(0, s_y): # s_x po 1, x_y po 2
            if enemy_ships1[j][i] > 0:
                color = "aqua"
                if points1[j][i] != -1:
                    color = "red"  # если кликнули и попали зеленый цветвв
...
def button_begin_again(): # почати спочатку + обнуление списковввввв
    global list_ids
    ...
    for el in list_ids:
        canvas.delete(el)
    list_ids = []
    # удаляет все елементы, корабли
    generate_ships_list() # заново генерирует
    # print(ships_list)
...
b2 = Button(tk, text="Почати спочатку!", command=button_begin_again)
...
def check_winner(x, y):
    ...
    if sum_enemy_ships1 == sum_boom: # если сумма списка кораблей равно списку бум, то это вин + обновление списка нужно бум
        win = True
    return win
...
def hod_computer():  # Ход компьютеру
    global points1, points2, hod_igrovomu_polu_1
    tk.update()
    time.sleep(1)
    hod_igrovomu_polu_1 = False
    ip_x = random.randint(0, s_x-1)  # Компьютер грая на рандом
    ...
    if check_winner2():
        ...
        winner_add = "(Усі кораблі противника гравця №1 переможені)!!!!!"
...
def add_to_all(event): # привязка клику к полю, для создания крестиков, кружков при попаданию
    global points1, points2, hod_igrovomu_polu_1
    _type = 0  # ЛКМ
    if event.num == 3:
        _type = 1  # ПКМ
    # print(_type)          ПОЛУЧЕНИЕ КООРДИНАТ ПРИ КЛИКЕ ПО ПОЛЮ лкм И пкм
    ...
    # первое игровое поле
    if ip_x < s_x and ip_y < s_y and hod_igrovomu_polu_1: # при клике отрисовка
        if points1[ip_y][ip_x] == -1:
            ...
            if check_winner2():
                ...
                winner_add = "(Усі кораблі противника Гравець №1 переможені)!!!!!"
                print(winner, winner_add)
                points1 = [[10 for i in range(s_x)] for i in range(s_y)]
                points2 = [[10 for i in range(s_x)] for i in range(s_y)]
                id1 = canvas.create_rectangle(step_x*3, size_canvas_y // 2, size_canvas_x + menu_x + size_canvas_x-step_x*3, size_canvas_y // 2+step_y+step_y // 2 + 50 + 25 + step_y // 2, fill="yellow") # создания крестиков при попадании
                list_ids.append(id1)  # очистка
    ...
    if ip_x >= s_x + delta_menu_x and ip_x <= s_x + s_x + delta_menu_x and ip_y < s_y and not hod_igrovomu_polu_1:
        # print("ok")
        if points2[ip_y][ip_x - s_x - delta_menu_x] == -1: # если кликнули
            ...
            if check_winner2_igrok_2():
                hod_igrovomu_polu_1 = False
                winner = "Перемога Гравець №1"
                winner_add = "(Усі кораблі противника Гравець №2 переможені)!!!!!"
                print(winner, winner_add)
                points1 = [[10 for i in range(s_x)] for i in range(s_y)]
                points2 = [[10 for i in range(s_x)] for i in range(s_y)]
...
def generate_ships_list(): # создание и генерация кораблей
    global ships_list
    ships_list = []
...
def generate_enemy_ships(): # генирация кораблей случайным образов с фиксированными длинами через  список
    # Использует список кораблей для функции
    global ships_list
    enemy_ships = []

    # подсчет суммарной длины кораблей (important)
    ...
    # print("sum: ", sum_1_all_ships)
          ...
          # Ниже происходит разброс кораблей таким образом что бы они не пересекались друг с другом.
...
# смещения размещения с проверкой на границы и соседние координаты,
                ...
                if primerno_x + len <= s_x:
                    for j in range(0, len):
                        try:
                            ...
                            # (проверка на ближ корабль). матрица кораблей
...
# print(ships_list)
enemy_ships1 = generate_enemy_ships()
enemy_ships2 = generate_enemy_ships() # для 1 и 2 игрока генерит корабли
# print("****************************")
# print(enemy_ships1)
# print("****************************")
# print(enemy_ships2)
...
while app_running: # шаг 3 ігровий цикл
    if app_running:
        tk.update_idletasks()
        tk.update()
    time.sleep(0.005)